...
class HolidaysRequest(models.Model):
    ...
    def _get_calendar(self):
        self.ensure_one()
        # self.remove_last_saturday_holidays()
    ...
    def action_send_email(self):
        ...
        # dates = "{} to {}".format(self.request_date_from, self.request_date_to)
        #
        # if self.request_date_to == self.request_date_from:
        #     dates = "{}".format(self.request_date_from)
        #
        # body = template.body_html
        # body = body.replace('{{user}}', str(employee_id))
        # body = body.replace('{{dates}}', str(dates))
        ...
        # template = self.env.ref('hr_holidays.email_template_time_off_request')
        # template_id = self.env.ref('hr_holidays.email_template_time_off_approval').id
        # new_template = self.env['mail.template'].create(template_values)

        # new_template = self.env.ref('module_hr_holidays.email_template_time_off_approval')
        # new_template.email_to = 'divyajp@infusionanalysts.com'

        # mail_template = self.env['ir.model.data']._xmlid_lookup('module_hr_holidays.email_template_time_off_approval')
        # mail_template = self.env['ir.model.data'].xmlid_to_object('module_hr_holidays.email_template_time_off_approval')

        # mail_template = self.env['mail.template'].browse(mail_template_id)
        # template_id = ir_model_data._get_id('hr_leave','email_template_time_off_approval')
#
#     def _get_last_saturday(self, year, month):
#         last_day = datetime(year, month, 1) + relativedelta(months=1, days=-1)
#         last_saturday = last_day - timedelta(days=(last_day.weekday() + 1) % 7)
#         return last_saturday.date()
#
#     @api.depends('date_from', 'date_to')
#     def _compute_excluded_holidays(self):
#         for leave in self:
#             excluded_holidays = []
...
#                     last_saturday = self._get_last_saturday(current_date.year, current_date.month)
#                     if current_date == last_saturday:
#                         excluded_holidays.append((0, 0, {
#                             'holiday_date': current_date,
#                             'name': 'Excluded Holiday'
#                         }))
#                 current_date += timedelta(days=1)
#             leave.excluded_holidays = excluded_holidays
#
#     excluded_holidays = fields.One2many('hr.holidays.excluded',  'leave_id', compute='_compute_excluded_holidays',
#                                         store=True)
# class HrHolidaysExcluded(models.Model):
#     _name = 'hr.holidays.excluded'
#
